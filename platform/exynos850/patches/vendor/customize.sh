...
if [[ "$TARGET_CODENAME" != "a21s" ]]; then
    ADD_TO_WORK_DIR "a21snsxx" "vendor" "bin/hw/vendor.samsung.hardware.light-service"
    ADD_TO_WORK_DIR "a21snsxx" "vendor" "lib64/android.hardware.light@2.0.so"
    ADD_TO_WORK_DIR "a21snsxx" "vendor" "lib64/vendor.samsung.hardware.light-V1-ndk_platform.so"
else
